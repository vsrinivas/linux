...
static __init int svm_hardware_setup(void)
{
	...
	/*
	 * The IBS virtualization requires either AVIC or VNMI to be enabled.
	 * Since AVIC can be inhibited and uninhibited dynamically, VNMI must
	 * be enabled to enable IBS virtualization.
	 */
	if (vibs)
		vibs = boot_cpu_has(X86_FEATURE_VIBS) && enable_passthrough_pmu
			&& vnmi;
